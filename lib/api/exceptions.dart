...
class UnwantedResponse extends APIError {
  ...
  Map<String, dynamic> get body =>
      response.body.isEmpty ? {} : api_common.decodeBody(response);
  @override
  String toString() =>
      '$runtimeType: $statusCode ${response.reasonPhrase}${body.isEmpty ? '' : ' - $body'}';
}

class FailedRequest extends APIError {
  final String message;
  final Uri? uri;
  FailedRequest(http.ClientException e)
      : message = e.message,
        uri = e.uri;
  @override
  String toString() =>
      '$runtimeType: $message, ${null == uri ? '' : 'uri=$uri'}';
}
